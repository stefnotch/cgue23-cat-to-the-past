...
trait FromMyEnum {
    fn from_enum(value: &MyEnum) -> Option<&Self>
    where Self: Sized;
}
impl FromMyEnum for Foo {
    fn from_enum(value: &MyEnum) -> Option<&Self> {
        match value {
            MyEnum::Foos(v) => Some(&v),
            _ => None
        }
    }
...
impl Cont {
    fn get_all<T>(&self) -> Vec<&T> where T : FromMyEnum {
        let mut result = Vec::new();
        self.values.iter().for_each(|child| {
            if let Some(value) = T::from_enum(child) {
                result.push(value);
            }
        });
        result
...
fn main() {
    let e = Cont{values: vec![MyEnum::Foos(Foo), MyEnum::Bars(Bar), MyEnum::Foos(Foo)]};
    let ee = e.get_all::<Foo>();
    let ee2 = e.get_all::<Bar>();
    //dbg!(e);
    dbg!(ee);
}
 */
